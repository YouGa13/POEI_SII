...
int main()
{
       char liste[][30] = { "abolir" };
       char mask[][30] = { determiner_c_a_deviner(liste) }; 

       char  nom[20], c_lenght;
       int i, indice_mot, nbre_c_manquant, essai, nbre_c_valide;
       srand(time(NULL));
       
       indice_mot = nbAleatoire(0, NBRE_D_INDICE); // je récupère l'indice du mot à déviner dans les différents tableaux
       ...
       nbre_c_manquant = compter_c_manquant_dans_une_chaine(mask[indice_mot]);
       // Debut du jeu
...
char* determiner_c_a_deviner(char chaine[NBRE_D_INDICE][30]) {
    if (chaine == NULL) return NULL;
    int nbre = 0;
    int i = 0;
        while (i <= NBRE_C_A_DEVINER) {
            nbre = nbAleatoire(0, NBRE_C_A_DEVINER);
            for (int j = 0; j <= NBRE_D_INDICE; j++) {
                if ((chaine[j]) && (chaine[j][nbre] != '_')) {
                    chaine[j][nbre] = '_';
                    i++;
                }
            }
...
int compter_c_manquant_dans_une_chaine(char* chaine) {
    int nbre = 1;
    for (int i = 0; chaine[i] != '\0'; i++)
    {
        if (chaine[i] == '_')
        {
            nbre++; // je compte le nombre de caractère à déviner
        }
    }
    return nbre;
}
